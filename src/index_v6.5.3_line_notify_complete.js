...
async function runLineNotifications(
  ...
  for (
    ...
    const existing =
      await getLineNotificationState(
        env,
        pick.raceKey
      );
...
(function(){
  ...
  function render(data){
    ...
    list.innerHTML = picks.map(function(p){
      ...
      var result = '';
      if(p.resultCheck){
        var hit = !!p.resultCheck.hit;
        result = '<div class="result ' + (hit ? 'hit' : 'miss') + '">' +
          (hit ? '✅ 的中' : '結果確定') + '：' + esc(p.resultCheck.combination || '-') +
          (p.resultCheck.payout != null ? ' / ' + esc(p.resultCheck.payout) + '円' : '') + '</div>';
      }
